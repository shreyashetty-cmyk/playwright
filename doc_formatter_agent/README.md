...
### Research Agent (Browser Research → Formatted Docx)

Given a topic, the research agent searches the web, opens top articles, extracts text, builds a Word document, and formats it using your existing backend.

**One command (starts backend automatically):**

```bash
cd agent
python run_all.py --research "Impact of AI in Intelligence Operations"
```

**Or run research only (backend must already be running):**

From **project root** (recommended):

```bash
python run_research.py "Climate change effects on agriculture"
python run_research.py "Your topic" --articles 5 --duckduckgo
```
...
```bash
cd agent
python research_agent.py "Climate change effects on agriculture"
python research_agent.py "Your topic" --articles 5 --duckduckgo
```

Output: `agent/research_output.docx` (formatted with your existing formatter).
...
doc_formatter_agent/
├── agent/
│   ├── run_all.py          # Main script (format or research)
│   ├── run_agent.py        # Playwright doc format automation
│   ├── research_agent.py   # Browser research agent (search → extract → docx → format)
